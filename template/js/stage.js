...
async function search_gold() {
	...
	for (let i = 0, I = M1.children.length; i < I; ++i) {
		...
		if (info1.dataset.hasOwnProperty('forbidGoldComp')) {
...
async function render_stage() {
	...
	if (info3[SI_FLAGS] || info2[SM_RESETMODE] || info1.dataset.hasOwnProperty('forbidGoldComp') || info2[SM_WAITFORTIMER] || info2[SM_FLAGS] || info2[SM_SPECIALCOND]) {
		var s;
		var tr = stName.parentNode.parentNode.appendChild(document.createElement("tr"));
		tr.id = "warn-tr";
		...
		if (info1.dataset.hasOwnProperty('forbidGoldComp') || flags2 & 2) {
			var span = th.appendChild(document.createElement("div"));
			span.classList.add('W');
